...
import { success, error, section } from "../utils/display.js";
  ...
  program
    ...
    .action(async () => {
      ...
      const { metaAccessToken } = await inquirer.prompt<{ metaAccessToken: string }>([
        {
          ...
          validate: (input: string) =>
            input.length > 0 ? true : "Access token is required.",
        },
      ]);
      ...
      const goals = await inquirer.prompt<{
      ...
      }>([
        {
          ...
          validate: (input: number) =>
            input > 0 ? true : "ROAS target must be positive.",
        },
        {
          type: "number",
          name: "cpaCap",
          ...
          default: 25.0,
          validate: (input: number) =>
            input > 0 ? true : "CPA cap must be positive.",
        },
        {
          type: "number",
          ...
          default: 500,
          validate: (input: number) =>
            input > 0 ? true : "Budget limit must be positive.",
        },
        {
          type: "list",
          name: "riskLevel",
      ...
      if (tokenValid) {
        ...
        error(
          "Could not validate the Meta access token. Please check your token and try again.",
        );
      }
