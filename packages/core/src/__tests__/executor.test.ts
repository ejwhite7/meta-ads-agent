...
import { ToolExecutionError, createTool } from "../tools/types.js";
...
function createTestTool(
	executeFn: () => Promise<{ success: boolean; data: null; message: string }>,
) {
	return createTool({
		name: "test_tool",
...
describe("ToolExecutor", () => {
	...
	it("should throw ToolExecutionError after all attempts fail", async () => {
		...
		await expect(executor.execute("test_tool", { value: "test" }, mockContext())).rejects.toThrow(
			ToolExecutionError,
		);
	...
	it("should throw when tool is not registered", async () => {
		...
		await expect(executor.execute("nonexistent_tool", {}, mockContext())).rejects.toThrow(
