...
export const scaleCampaignTool = createTool({
  ...
  parameters: Type.Object({
    ...
    reason: Type.String({
      description:
        "Why the budget is being scaled — logged to the audit trail",
    }),
  }),
    ...
    try {
      ...
      if (scaleFactor > guardrails.maxBudgetScaleFactor) {
        return {
          success: false,
          error:
            `Scale factor ${scaleFactor} exceeds maximum allowed ` +
            `${guardrails.maxBudgetScaleFactor}. Reduce the scale factor ` +
            `or adjust guardrail configuration.`,
      ...
      const message =
        error instanceof Error ? error.message : "Unknown error scaling campaign";
