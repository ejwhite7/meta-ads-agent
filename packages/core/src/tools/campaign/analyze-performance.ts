...
import type { CampaignInsights } from "../../types.js";
...
export const analyzePerformanceTool = createTool({
	...
	parameters: Type.Object({
		...
		dateRange: Type.Union(
			[Type.Literal("last_3d"), Type.Literal("last_7d"), Type.Literal("last_30d")],
			{
				default: "last_7d",
		...
		try {
			...
			if (!campaign.insights) {
				return {
					success: false,
					data: null,
					error: `Campaign ${campaignId} has no performance data available. The campaign may be newly created or have zero delivery.`,
					message: `Campaign ${campaignId} has no performance data available. The campaign may be newly created or have zero delivery.`,
			...
			const roasGapPercent = roasTarget > 0 ? roasGap / roasTarget : 0;
			...
			const cpaGapPercent = cpaCap > 0 ? cpaGap / cpaCap : 0;
			...
			const message =
				error instanceof Error ? error.message : "Unknown error analyzing campaign performance";

			return {
				success: false,
				data: null,
				error: `Failed to analyze campaign ${campaignId}: ${message}`,
				message: `Failed to analyze campaign ${campaignId}: ${message}`,
