...
export const AgentConfigSchema = z.object({
  ...
  metaAdAccountId: z
    .string()
    .describe('Meta ad account ID to manage'),

  /** Meta access token for API authentication */
  metaAccessToken: z
    .string()
    .describe('Meta Marketing API access token'),
  ...
  maxIterationsPerRun: z
    .number()
    .int()
    .positive()
    .default(24)
    .describe('Max iterations per run'),
  ...
  retryBackoffMs: z
    ...
    .positive()
    .default(5_000)
    .describe('Base retry backoff in ms'),
  ...
  dryRun: z
    .boolean()
    .default(false)
    .describe('Dry run mode — log without executing'),

  /** Database type: "sqlite" for local dev, "postgres" for cloud (default: "sqlite") */
  dbType: z
    .enum(['sqlite', 'postgres'])
    .default('sqlite')
    .describe('Database backend type'),
