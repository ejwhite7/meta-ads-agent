...
export class AgentSession {
  ...
  getStatus(): SessionStatus {
    const nextTickAt = this.state === 'running' && this.lastTickAt
      ? new Date(
  ...
  private scheduleTick(): void {
    ...
    const backoff = this.consecutiveFailures > 0
      ? Math.min(baseInterval, this.sessionConfig.config.retryBackoffMs * Math.pow(2, this.consecutiveFailures - 1))
      : 0;
